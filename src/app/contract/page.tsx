...
export const metadata: Metadata = {
  ...
  description:
    "Мы работаем по индивидуальным договорам. Оплата принимается только после подписания договора по СБП. Прозрачность и защита ваших интересов.",
  robots: "index, follow",
};
...
export default function ContractPage() {
  ...
  return (
    ...
    >
      ...
      {/* Секции */}
      {contractContent.sections.map(
        (section: { title: string; content: string }, index: number) => (
          <LegalSection key={index} title={section.title} content={section.content} />
        ),
      )}

      {/* Call to Action */}
      <div className="mt-12 bg-linear-to-r from-blue-500 to-blue-600 rounded-lg p-8 text-white">
        <h2 className="text-2xl font-bold mb-3">{contractContent.cta.title}</h2>
        <p className="text-blue-50 mb-6 text-lg">{contractContent.cta.description}</p>
        <Link
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-800 dark:text-yellow-200">
              Мы никогда не просим внести оплату до подписания официального договора. Если кто-то
              просит вас оплатить услуги без договора от нашего имени — это мошенники. Свяжитесь с
              нами для проверки.
            </p>
          </div>
        </div>
