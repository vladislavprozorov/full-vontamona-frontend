...
import type { Benefit, BenefitsSectionProps } from "./benefits.types";
      ...
      <Card className="group h-full flex flex-col border-neutral-900/8 bg-white relative overflow-hidden transition-all duration-700 ease-out hover:shadow-[0_30px_90px_rgba(0,0,0,0.08)] before:absolute before:left-0 before:top-0 before:h-full before:w-[3px] before:bg-gradient-to-b before:from-primary/40 before:via-primary/20 before:to-transparent before:opacity-60">
        ...
        <CardContent className="relative p-9 md:p-11 flex flex-col h-full">
          ...
          <div className="h-14 w-14 rounded-2xl border border-neutral-900/8 flex items-center justify-center mb-7 bg-gradient-to-br from-white to-neutral-50/50 shadow-sm transition-all duration-700 group-hover:shadow-md group-hover:border-neutral-900/12">
            <div className="text-primary [&>svg]:w-7 [&>svg]:h-7">{benefit.icon}</div>
          </div>
          ...
          {/* DESCRIPTION */}
          <p className="mb-7 text-[15px] leading-[1.7] text-neutral-600">{benefit.description}</p>
          ...
          <div className="mb-auto">
            <p className="text-[14px] text-neutral-800 leading-[1.7] border-l-2 border-primary/30 pl-4">
              Один человек. Один контакт.{" "}
              <span className="text-neutral-900 font-medium">Полный контроль.</span>
            </p>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-neutral-900/5">
            <p className="text-[11px] text-neutral-400 leading-relaxed">
              Работаем напрямую с <span className="text-neutral-600 font-medium">MSC Cruises</span>{" "}
              и другими премиальными линиями
            </p>
          </div>
        </CardContent>
      </Card>
...
function SecondaryBenefitCard({ benefit, isLast }: { benefit: Benefit; isLast?: boolean }) {
  return (
    <motion.div variants={itemVariants} className={isLast ? "md:pr-6" : ""}>
      <Card
        className={`group h-full flex flex-col border-neutral-900/6 relative overflow-hidden transition-all duration-700 ease-out hover:shadow-[0_20px_60px_rgba(0,0,0,0.05)] hover:translate-y-[-2px] ${
          isLast
      ...
      >
        <CardContent className={isLast ? "p-7 flex flex-col h-full" : "p-8 flex flex-col h-full"}>
          ...
          <div
            className={`rounded-xl border border-neutral-900/6 flex items-center justify-center bg-gradient-to-br from-white to-neutral-50/30 transition-all duration-700 group-hover:border-neutral-900/10 ${
              isLast ? "h-9 w-9 mb-4" : "h-11 w-11 mb-5"
            }`}
          >
            <div
              className={`text-primary ${isLast ? "[&>svg]:w-4 [&>svg]:h-4" : "[&>svg]:w-5 [&>svg]:h-5"}`}
            >
              {benefit.icon}
            </div>
          </div>
          ...
          <h3
            className={`font-semibold tracking-tight text-neutral-900 ${
              isLast ? "text-[16px] leading-[1.3] mb-2.5" : "text-[18px] leading-[1.2] mb-3"
            }`}
          >
            {benefit.title}
          </h3>
          ...
          <p
            className={`leading-[1.7] text-neutral-600 mb-auto ${
...
export function BenefitsSection({ className = "" }: BenefitsSectionProps) {
  ...
  return (
    <section
      className={`relative py-24 md:py-32 bg-gradient-to-b from-white via-neutral-50/30 to-white overflow-hidden ${className}`}
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6">
        ...
        >
          ...
          <p className="mt-3 text-[12px] text-neutral-500">Ответим в течение 2 часов</p>
        </motion.div>
        ...
        >
          <p className="text-[12px] text-neutral-400 leading-relaxed max-w-xl mx-auto">
            Нам доверяют клиенты, которые ценят{" "}
            <span className="text-neutral-600 font-medium">спокойствие</span> и{" "}
            <span className="text-neutral-600 font-medium">безупречный сервис</span>
          </p>
        </motion.div>
      </div>
    </section>
